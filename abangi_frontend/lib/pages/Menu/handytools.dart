...
class HandyToolsScreen extends StatefulWidget {
  const HandyToolsScreen({Key? key}) : super(key: key);
...
class _MyStatefulWidgetState extends State<HandyToolsScreen> {
  ...
  final List<ItemModel> HandyToolsData = List.generate(
      handytools.length,
      (index) => ItemModel(handytools[index], 'Description', url[index],
  ...
  Widget build(BuildContext context) {
    return Padding(
        ...
        child: ListView.builder(
            itemCount: HandyToolsData.length,
            itemBuilder: (context, index) {
              return Card(
                child: ListTile(
                  leading: Image.network(HandyToolsData[index].itemImage),
                  title: Text(HandyToolsData[index].itemName),
                  subtitle: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(HandyToolsData[index].itemPrice,
                          style: TextStyle(
                              color: Color.fromRGBO(0, 176, 236, 1),
                              fontWeight: FontWeight.bold,
                              fontSize: 15)),
                      Text(HandyToolsData[index].itemLocation),
                      Text(HandyToolsData[index].itemOwner),
                    ],
                  ),
                ),
              );
